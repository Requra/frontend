  ...
  ({ className, type, label, description, error, wrapperClassName, startIcon, endIcon, id, ...props }, ref) => {
    ...
    const inputElement = startIcon || endIcon ? (
      <div className="relative w-full">
        {startIcon && (
          <div className="absolute left-2.5 top-1/2 -translate-y-1/2 text-neutral-400 pointer-events-none flex items-center justify-center [&>svg]:h-4 [&>svg]:w-4">
            {startIcon}
          </div>
        )}
        {baseInput}
        {endIcon && (
          <div className="absolute right-2.5 top-1/2 -translate-y-1/2 text-neutral-400 pointer-events-none flex items-center justify-center [&>svg]:h-4 [&>svg]:w-4">
            {endIcon}
          </div>
        )}
      </div>
